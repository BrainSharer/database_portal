...
from brain.models import Animal, Slide, ImageFile
...
def repeat_scene(slide_id, inserts, scene_number):
    tifs = ImageFile.objects.filter(slide_id=slide_id).filter(active=True) \
...
def remove_scene(slide_id, deletes, scene_number):
    channels = ImageFile.objects.filter(slide_id=slide_id).filter(active=True).values('channel').distinct().count()
    tifs = ImageFile.objects.filter(slide_id=slide_id).filter(active=True) \
        .filter(scene_number=scene_number)
...
def find_closest_neighbor(slide_id, scene_number):
    ...
    """
    channels = ImageFile.objects.filter(slide_id=slide_id).filter(active=True).values('channel').distinct().count()
    below = ImageFile.objects.filter(slide_id=slide_id).filter(active=True) \
...
def set_scene_active(slide_id, scene_number):
    channels = ImageFile.objects.filter(slide_id=slide_id).filter(active=True).values('channel').distinct().count()
    active_tifs = ImageFile.objects.filter(slide_id=slide_id)\
        .filter(active=True).filter(scene_number=scene_number).order_by('scene_number')[:channels]
    inactive_tifs = ImageFile.objects.filter(slide_id=slide_id)\
...
def set_scene_inactive(slide_id, scene_number):
    tifs = ImageFile.objects.filter(slide_id=slide_id).filter(active=True).filter(scene_number=scene_number)
    for tif in tifs:
        tif.active = False
        tif.save()
...
def set_end(slide_id, scene_number):
    tifs = ImageFile.objects.filter(slide_id=slide_id).filter(scene_number__gte=scene_number)
    for tif in tifs:
        tif.active = False
        tif.save()
...
def scene_reorder(slide_id):
    ...
    scenes_tifs = ImageFile.objects.filter(slide_id=slide_id).filter(active=True).order_by('scene_number')
    channels = ImageFile.objects.filter(slide_id=slide_id).filter(active=True).values('channel').distinct().count()
...
def save_slide_model(self, request, obj, form, change):
    ...
    obj.scenes = ImageFile.objects.filter(slide_id=obj.id).filter(channel=1).filter(active=True).count()
...
class TifInlineFormset(forms.models.BaseInlineFormSet):
    ...
    def save_existing(self, form, instance, commit=True):
        ...
        ch23s = ImageFile.objects.filter(slide_id=obj.slide_id).filter(scene_number=obj.scene_number).filter(scene_index=obj.scene_index)
        for ch23 in ch23s:
            ch23.active = False
            ch23.save()
